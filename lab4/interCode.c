...
void printInterCode(InterCode* p){
		switch(p->kind){
			...
			case ASSIGN_CODE:
				...
				break;
			case ADD_CODE:
				printOp(p->u.doubleop.result);
				printf(":= ");
				printOp(p->u.doubleop.op1);
				printf("+ ");

				printOp(p->u.doubleop.op2);
				break;
			case SUB_CODE:
				printOp(p->u.doubleop.result);
				printf(":= ");
			...
			case IF_GOTO_CODE:
				printf("IF ");
				printOp(p->u.tribleop.x);
				printf("%s",p->u.tribleop.relop);
				printf(" ");
				printOp(p->u.tribleop.y);
				printf("GOTO ");
				printOp(p->u.tribleop.gotoLabel);
				break;
			case RETURN_CODE:
				printf("RETURN ");
				printOp(p->u.singleop.op);
