...
int sizeofType(SymDes* type){
	if(type->kind == BASIC_KIND){
		if(type->detail.basic == INT_TYPE)
			return INT_SIZE;
		else if(type->detail.basic == FLOAT_TYPE)
			return FLOAT_SIZE;
		else
			return -1;
	}
	else if(type->kind == STRUCT_KIND){
		int size = 0;
		SymDes* fieldList = type->detail.struc->fieldList;
		while(fieldList!=NULL){
			size += sizeofType(fieldList);
			fieldList = fieldList->nextInList;
		}
		return size;
	}
	else if(type->kind ==ARRAY_KIND){
		return type->detail.array.size*sizeofType(type->detail.array.formBy);
	}
	return -1;
}
...
void initTable(){
	...
	//向符号表里插入read和write函数
	FuncDes* readFunc = malloc( sizeof(FuncDes) );
	memset(readFunc, 0, sizeof(FuncDes));
	char readFuncName[] = "read";
	readFunc->name = malloc(sizeof(readFuncName));
	strcpy(readFunc->name, readFuncName);
	readFunc->isDef = TRUE;

	SymDes* readFuncRet = malloc(sizeof(SymDes));
	memset(readFuncRet, 0, sizeof(SymDes));
	readFuncRet->kind = BASIC_KIND;
	readFuncRet->detail.basic = INT_TYPE;
	readFunc->retType = readFuncRet;

	insertFuncTable(readFunc);
	
	FuncDes* writeFunc = malloc( sizeof(FuncDes) );
	memset(writeFunc, 0, sizeof(FuncDes));
	char writeFuncName[] = "write";
	writeFunc->name = malloc(sizeof(writeFuncName));
	strcpy(writeFunc->name, writeFuncName);
	writeFunc->isDef = TRUE;

	SymDes* writeFuncRet = malloc(sizeof(SymDes));
	memset(writeFuncRet, 0, sizeof(SymDes));
	writeFuncRet->kind = BASIC_KIND;
	writeFuncRet->detail.basic = INT_TYPE;
	writeFunc->retType = writeFuncRet;

	SymDes* writeParam = malloc(sizeof(SymDes));
	memset(writeParam, 0, sizeof(SymDes));
	char writeParamName[] = "writeFuncParam";
	writeParam->name = malloc(sizeof(writeParamName));
	strcpy(writeParam->name, writeParamName);
	writeParam->kind = BASIC_KIND;
	writeParam->detail.basic = INT_TYPE;
	writeFunc->paraList = writeParam;
